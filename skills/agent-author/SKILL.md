...
- [ ] Everywhere: Add a worked example when ambiguity or error risk warrants it, especially where a section stacks 3+ interacting technical facts (e.g. permission/ordering/parsing rules); apply the cheap-model test from core rules to that section specifically, not just the artifact as a whole.
