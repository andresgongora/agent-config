...
`template/script.sh` is the canonical starter. Every `TEMPLATE:` marker is an authoring instruction: remove or replace it, and every example, before running ShellCheck. It contains exactly one example core function and one placeholder dependency — enough to show the pattern, not enough to clutter.

`template/snippets.sh` is opt-in. Copy only required blocks: `basename`/`dirname` path helpers, root checks, timestamps, logging. Not standalone. `logVerbose` stays silent until `VERBOSE=true`.

## Linting configs

`template/.shellcheckrc` and `template/.editorconfig` are drop-in files. Copy both to a script project root.
